...
(
    ...
    `comment_made` DATETIME NOT NULL,
    `active` TINYINT(1),
...
DROP TABLE IF EXISTS `comments_deleted`;
...
(
    ...
    `comment` VARCHAR(1000) NOT NULL,
    `comment_made` DATETIME NOT NULL,
    `comment_deleted` DATETIME NOT NULL,
    PRIMARY KEY (`comment_id`),
    INDEX `fkdc_made_by_idx` (`made_by_user`),
    INDEX `fkdc_post_id_idx` (`to_post`),
    CONSTRAINT `fkc_made_by`
        FOREIGN KEY (`made_by_user`)
        REFERENCES `users` (`email`),
    CONSTRAINT `fkc_post_id`
        FOREIGN KEY (`to_post`)
        REFERENCES `posts` (`post_id`)
) ENGINE=InnoDB;
